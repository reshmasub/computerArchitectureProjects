...
//debug_l2 cout << "Cache Configuration:" <<endl;
//debug_l2 cout << "CACHE_SIZE:      " << CACHE_SIZE << endl;
//debug_l2 cout << "CACHE_LINE_SIZE: " << CACHE_LINE_SIZE << endl;
//debug_l2 cout << "ASSOCIATIVITY:   " << ASSOCIATIVITY << endl;
    }
...
//debug_l2 cout << "*******************************************************************************************************************" << endl;
//debug_l2 cout << "Read from the addr_trace and data_size files. INSTRUCTION ADDR in hex: 0x" << hex << addr << endl;
//debug_l2 cout <<" Data size to be read from instruction cache in decimal: " << dec << data_size_per_address << endl;
...
//debug_l2 cout << "Hit found but the prev addr index was the same as addr index" << endl;
                }
//debug_l2 cout << "DEBUG: " << cache_data[addr_index+hit_in_set*CACHE_INDEX_DEPTH][addr_byte_offset/2] << endl;
                ...
                if(valid_zero_found == 0x1) {
//debug_l2 cout << "Valid zero found. Associativity is " << ASSOCIATIVITY << endl;
...
//debug_l2 cout << "Valid zero not found." << endl;
...
//debug_l2 cout << "DEBUG: " << cache_data[addr_index+rand_set_to_write*CACHE_INDEX_DEPTH][addr_byte_offset/2] << endl;
           ...
           // cout << "DEBUG:!!!!!!!!! Outside the check_cache function. valid_match_arr[0] = " << valid_match_arr[0] << endl;
            ...
            /*
            //On getting the address from the address file check the cache
            if(cache_valid[addr_index] == 0x1) {  //Valid is TRUE
                if(addr_tag == cache_tag[addr_index]) { //TAG is a MATCH
//debug_l2 cout << "Valid is 0x" << hex << cache_valid[addr_index] << endl;
//debug_l2 cout << "Expected Tag is 0x" << hex << addr_tag << ". Actual tag is 0x" << hex << cache_tag[addr_index]<< endl;
//debug_l2 cout << "The cache access for addr: 0x" << hex << local_addr << " is a HIT!!!" << endl;
                    
                    if(num_acc_per_addr == 0x0 || (prev_addr_index != addr_index)) {
                        hit = hit + 1;
                        num_of_clock_cycles  = num_of_clock_cycles + 1;
                    }
                    found_in_cache = 1;
//debug_l2 cout << "DEBUG: " << hex << cache_data[addr_index][addr_byte_offset/2] << endl;
                    data_output = cache_data[addr_index][addr_byte_offset/2]+data_output;
                } else {                    //TAG isnt a MATCH
//debug_l2 cout << "Expected Tag is 0x" << hex << addr_tag << ". Actual tag is 0x" << hex << cache_tag[addr_index]<< endl;
//debug_l2 cout << "The cache access for addr: 0x" << hex << local_addr << " is a MISS.!!!" << endl;
                    
                    cache_tag[addr_index] = addr_tag;
                    cache_valid[addr_index] = 0x1;
                    cacheline = ReadMem(halfword, local_addr, CACHE_LINE_SIZE);
                    cache_data[addr_index] = cacheline;
...
//debug_l2 cout << "Number of Misses: " << dec << miss << endl;
    ...
    /*
    ifstream datain;  //File pointer
    ifstream dataout;  //File pointer
    datain.open("inst_data_trace_hex_project_1.txt");    //Open the mem trace file.
    dataout.open("inst_data_trace_hex_project_1_result.txt");    //Open the mem trace file.

    string data_in, data_out;
    if (datain.is_open() && dataout.is_open()) {
    int line_num=1;
    
    while(getline(datain,data_in) && getline(dataout,data_out)) {
        if(data_in.compare(data_out) != 0) {
//debug_l2 cout << "Mismatch at line: "<< line_num << " Data in: 0x" << data_in << " Data out: 0x" << data_out << endl;
//debug_l2 cout << data_in.size() << data_out.size() <<endl;
        }
        line_num++;
    }
    }
